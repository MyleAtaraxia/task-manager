...
import { sql } from "bun";
import { SqlTaskStorage } from "./task_storage";

const hasDatabase = Boolean(process.env.DATABASE_URL);

describe.skipIf(!hasDatabase)("SqlTaskStorage", () => {
	let storage: SqlTaskStorage;

	beforeEach(async () => {
		storage = new SqlTaskStorage();
		await storage.init();
	...
	test("getTask returns the stored task, or null when missing", async () => {
		...
		expect(
			await storage.getTask("00000000-0000-0000-0000-000000000000"),
		).toBeNull();
	});
	...
	test("updateTask applies a partial update and bumps updated_at, or returns false when missing", async () => {
		...
		expect(await storage.updateTask(created.id, { status: "done" })).toBe(
			true,
		);
		...
		expect(
			await storage.updateTask("00000000-0000-0000-0000-000000000000", {
				status: "done",
			}),
		).toBe(false);
	});
